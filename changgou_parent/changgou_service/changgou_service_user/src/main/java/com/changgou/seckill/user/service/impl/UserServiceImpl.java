package com.changgou.seckill.user.service.impl;

import com.alibaba.fastjson.JSON;
import com.changgou.seckill.order.pojo.Task;
import com.changgou.seckill.user.dao.PointLogMapper;
import com.changgou.seckill.user.dao.UserMapper;
import com.changgou.seckill.user.pojo.PointLog;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)     //因为方法【updateUserPoints】需要在Transactional注解指定rollbackFor或者在方法中显式的rollback。
    //    Inspection info:
    //    事务场景中，抛出异常被catch后，如果需要回滚，一定要手动回滚事务。
    public int updateUserPoints(Task task) {
        //1.从task中获取相关数据
        System.out.println("任务用户服务现在开始对任务进行处理");
